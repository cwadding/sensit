  ...
  class FeedsController < ApiController
    ...
    def create
      if params.has_key?(:feeds)
        ...
        @fields = topic.fields.load.to_a
        importer = Topic::Feed::Importer.new({index: elastic_index_name, type: elastic_type_name, :topic_id => topic.id, :fields => @fields, :feeds => feeds_params(@fields.map(&:key))})
        ...
        topic = Topic.find(params[:topic_id])
        @feed = Topic::Feed.new(feed_params(topic.fields).merge!({index: elastic_index_name, type: elastic_type_name, :topic_id => topic.id})) 
        if @feed.save
