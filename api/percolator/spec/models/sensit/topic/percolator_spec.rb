...
module Sensit
  describe Topic::Percolator do
    ...
    describe ".count" do
        context "when the index exists" do
            it "executes the elastic count query" do                
                @client.should_receive(:count).with({index: @user.to_param, type: "#{@user.to_param}:atm"}).and_return({"count"=>4, "_shards"=>{"total"=>1, "successful"=>1, "failed"=>0}})
                Topic::Percolator.stub(:elastic_client).and_return(@client)
                total = Topic::Percolator.count({topic_id: 'atm', user_id: @user.to_param})
            end
            it "returns the number of rows" do
                @client.stub(:count).with({index: @user.to_param, type: "#{@user.to_param}:atm"}).and_return({"count"=>36, "_shards"=>{"total"=>5, "successful"=>5, "failed"=>0}})
                Topic::Percolator.stub(:elastic_client).and_return(@client)
                total = Topic::Percolator.count({topic_id: 'atm', user_id: @user.to_param})
                total.should == 36
            end
    ...
    describe ".find" do
        context "with valid params" do
            context "when the record exists" do
                before(:each) do
                    @params = {topic_id: 'atm', user_id: @user.to_param, name: "myrule", query: { query: { query_string: { query: 'foo' } } } }
                ...
                it "executes the elastic get query" do
                    @client.should_receive(:get).with({index: @user.to_param, type: "#{@params[:user_id]}:#{@params[:topic_id]}", id:@params[:name], query: { query: { query_string: { query: 'foo' } } }}).and_return(@result1)
            ...
            context "when the record doesn't exist" do
                before(:each) do
                    @params = {topic_id: 'atm', user_id: @user.to_param, name: 'cOsusCVJQUabbfZcIdDyAg' }
        ...
        context "with invalid params" do
            before(:each) do
                @params = {topic_id: "fsd", user_id: @user.to_param , name: 'cOsusCVJQUabbfZcIdDyAg' }
                @client.stub(:get).and_raise(::Elasticsearch::Transport::Transport::Errors::NotFound)
            end
            it "throws an exception that it is not found" do
                Topic::Percolator.stub(:elastic_client).and_return(@client)
    ...
    describe ".create" do
        before(:each) do
            @params = {topic_id: 'mytype', user_id: @user.to_param, name: "myrule", query: { query: { query_string: { query: 'foo' } } } }
    ...
    describe ".search" do
        before(:each) do
            @params =  {topic_id: 'mytype', user_id: @user.to_param }
        ...
        it "executes the elastic search query" do
            @client.should_receive(:search).with(@params.merge!({index: @user.to_param, type: "#{@params[:user_id]}:#{@params[:topic_id]}"})).and_return(@results)
            Topic::Percolator.stub(:elastic_client).and_return(@client)
    ...
    describe ".destroy" do
        before(:each) do
            @params = {topic_id: 'mytype', user_id: @user.to_param, name: '1'}
        end
        context "when the record exists" do
            it "executes the elastic delete" do
                @client.should_receive(:delete).with({index: @user.to_param, type: "#{@params[:user_id]}:#{@params[:topic_id]}", id: @params[:name] }).and_return({"ok"=>true, "found"=>true, "_index"=>"transactions", "_type"=>"atm", "_id"=>"8eI2kKfwSymCrhqkjnGYiA", "_version"=>4})
                Topic::Percolator.stub(:elastic_client).and_return(@client)
        ...
        context "when the record does not exist" do
            it "throws an exception that it is not found" do
                @client.stub(:delete).with({index: @user.to_param, type: "#{@params[:user_id]}:#{@params[:topic_id]}", id: @params[:name] }).and_raise(::Elasticsearch::Transport::Transport::Errors::NotFound)
                Topic::Percolator.stub(:elastic_client).and_return(@client)
                expect{
                    Topic::Percolator.destroy(@params)
                }.to raise_error(::Elasticsearch::Transport::Transport::Errors::NotFound)
    ...
    describe ".destroy_all" do
        ...
        context "when the index exists" do
            it "executes the elastic index delete" do
                @indices_client.should_receive(:delete).with({index: @user.to_param, :type => "#{@user.to_param}:my_type"}).and_return({"ok"=>true, "acknowledged"=>true})
                @client.stub(:indices).and_return(@indices_client)
                Topic::Percolator.stub(:elastic_client).and_return(@client)
                Topic::Percolator.destroy_all({:topic_id => "my_type", :user_id => @user.to_param})
            end
        end
        context "when the index does not exist" do
            it "throws an exception that it is not found" do
                @indices_client.should_receive(:delete).with({index: @user.to_param, :type => "#{@user.to_param}:my_type"}).and_raise(::Elasticsearch::Transport::Transport::Errors::NotFound)
                @client.stub(:indices).and_return(@indices_client)
                Topic::Percolator.stub(:elastic_client).and_return(@client)
                expect{
                    Topic::Percolator.destroy_all({user_id: @user.to_param, :topic_id => "my_type"})
                }.to raise_error(Elasticsearch::Transport::Transport::Errors::NotFound)
            end
        end
    end 
    ...
    describe "#create" do
        ...
        before(:each) do
            @indices_client = Elasticsearch::API::Indices::IndicesClient.new(@client)
            @percolator = Topic::Percolator.new({topic_id: 'mytype', user_id: @user.to_param, name: 'myrule', query: { query: { query_string: { query: 'foo' } } } })
            # @params = {index: 'myindex', topic_id: 'mytype', user_id: @user.to_param, topic_name: 3, at: Time.now, values: {title: 'Test 1',tags: ['y', 'z'], published: true, counter: 1}}
            # {index: 'myindex',topic_id: 'mytype', user_id: @user.to_param, body: {title: 'Test 1',tags: ['y', 'z'], published: true, published_at: Time.now.utc.iso8601, counter: 1}}
        end
        ...
        context "when the index does exist" do
            ...
            context "with valid response" do
                it "executes the elastic create action" do
                    @client.should_receive(:create).with({index: @user.to_param, :type => "#{@user.to_param}:mytype", id: 'myrule', :body=>{ query: { query_string: { query: 'foo' } } }}).and_return({"ok"=>true, "_index"=>'_percolator', "_type"=>'mytype', "_id"=>"myrule", "_version"=>1})
                    Topic::Percolator.stub(:elastic_client).and_return(@client)
    ...
    describe "#update" do
        before(:each) do
            @percolator = Topic::Percolator.new({topic_id: 'mytype', user_id: @user.to_param, name: 3, query: {query: { query_string: { query: 'foo' } } }})
            @percolator.stub(:id).and_return(3)
            @percolator.stub(:new_record?).and_return(false)
        end
        it "executes the update class action" do
            @client.should_receive(:update).with({index: @user.to_param, type: "#{@user.to_param}:mytype", id: 3, body:{ doc:{ query: { query_string: { query: 'foo' } }}}}).and_return({"ok"=>true, "_index"=>'_percolator', "_type"=>'mytype', "_id"=>"myrule", "_version"=>1})
    ...
    describe "#destroy" do
        ...
        context "when the record is not a new record" do
            ...
            it "executes the elastic delete" do
                Topic::Percolator.should_receive(:destroy).with({user_id: @user.to_param, topic_id: "mytype", name: 3}).and_return(true)
